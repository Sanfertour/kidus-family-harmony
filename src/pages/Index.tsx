    ...
    return (
      <div className="relative min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center w-full max-w-sm">
          <img src="https://raw.githubusercontent.com/Sanfertour/kidus-family-harmony/main/src/assets/IMG_20260120_144903.jpg" className="w-40 h-40 mx-auto mb-8 rounded-[3rem] shadow-2xl shadow-sky-100" />
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-1 container mx-auto px-6 pt-6 max-w-md pb-48">
          ...
          <AnimatePresence mode="wait">
            {activeTab === "home" && <motion.div key="h" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><DashboardView onNavigate={setActiveTab} nextEvent={nextEventTitle} nestId={nestId} members={familyMembers} membersCount={familyMembers.length} /></motion.div>}
            {activeTab === "agenda" && <AgendaView key="a" />}
            {activeTab === "vault" && <VaultView key="v" nestId={nestId || ""} />}
            {activeTab === "settings" && <SettingsView key="s" nestId={nestId} members={familyMembers} onRefresh={() => fetchSession()} />}
          </AnimatePresence>
        </main>
        ...
        <ManualEventDrawer isOpen={isManualDrawerOpen} onClose={() => { setIsManualDrawerOpen(false); setAiExtractedData(null); }} initialData={aiExtractedData} members={familyMembers} onEventAdded={() => fetchSession()} />
