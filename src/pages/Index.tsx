...
const Index = () => {
  ...
  // Blindaje 1: Inicializamos siempre como un array vacío para evitar errores de .map()
  const [familyMembers, setFamilyMembers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const { toast } = useToast();

  // Función para cargar los miembros con manejo de errores robusto
  const fetchFamily = async () => {
    try {
      ...
      // Blindaje 2: Si data es null, ponemos array vacío
      setFamilyMembers(data || []);
    } catch (err) {
      console.error("Error cargando familia:", err);
      // No dejamos que sea undefined
      setFamilyMembers([]);
    } finally {
      setLoading(false);
    }
              ...
              {loading ? (
                ...
                familyMembers.map((member: any) => (
                  <div key={member.id} className="glass-card p-5 rounded-[2rem] flex flex-col items-center text-center group transition-all hover:scale-105">
                    ...
                    <span className="font-bold text-foreground block">{member.display_name || 'Sin nombre'}</span>
                ...
                <div className="col-span-2 glass-card p-8 text-center opacity-70">
                  ...
                  <p className="text-xs text-muted-foreground mt-1 text-balance">¡Añade a tu primer familiar para empezar!</p>
                </div>
              )}
            </div>
          </div>
