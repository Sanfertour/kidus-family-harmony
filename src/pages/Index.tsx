...
const Index = () => {
  ...
  useEffect(() => {
    ...
    // SUSCRIPCIÓN EN TIEMPO REAL: El alma de la sincronización entre apps
    const channel = supabase
      .channel('nido-compartido')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'events' }, () => {
        fetchAllData();
        toast({ 
          title: "Nido actualizado", 
          description: "La agenda se ha sincronizado con tu equipo.",
        });
      })
      ...
      <main className="container mx-auto px-6 pt-4 max-w-md relative z-10 pb-32">
        ...
        {activeTab === "home" && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="px-2">
              <h1 className="text-4xl font-black tracking-tight text-gray-800 leading-tight">
                Tu Nido está <br/> 
            ...
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-[3rem] blur opacity-25"></div>
              <div className="relative glass-card p-8 rounded-[3rem] bg-white/80 backdrop-blur-xl border border-white/50">
                ...
                <div className="mt-8 p-4 bg-gray-50/50 rounded-2xl flex items-center justify-between">
                  <div>
                  ...
                  <button className="px-4 py-2 bg-white rounded-xl shadow-sm text-xs font-black text-blue-600 active:scale-95">
                    MARCAR HECHO
                  </button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {familyMembers.map((member) => (
                <div key={member.id} className="glass-card p-6 flex flex-col items-center group transition-all hover:translate-y-[-4px]">
                  ...
                  <span className="text-[9px] font-bold text-blue-400 uppercase mt-1">{member.role || 'Miembro'}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="glass-card p-8 space-y-6 bg-indigo-50/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 font-bold">Un nido para dos pilotos</p>
                </div>
              </div>
              <div className="p-4 bg-white/80 rounded-2xl border-2 border-dashed border-indigo-200 text-center">
                ...
                <p className="text-2xl font-black tracking-[0.3em] text-indigo-600">KID-7721</p>
              </div>
              <div className="space-y-3">
                <input placeholder="Código de pareja..." className="w-full h-14 rounded-2xl bg-white border-none px-4 text-center font-black uppercase tracking-widest text-sm shadow-sm" />
                <Button className="w-full h-14 rounded-2xl bg-gray-800 text-white font-black">FUSIONAR NIDOS</Button>
              </div>
            </div>
            <Button variant="ghost" className="w-full justify-start h-14 rounded-2xl font-bold text-red-500" onClick={() => supabase.auth.signOut()}>
