...
import { X, Calendar, Clock, User, CheckCircle2, Lock, EyeOff } from 'lucide-react';
...
export const ManualEventDrawer = ({ 
  ...
  const [isPrivate, setIsPrivate] = useState(false); // NUEVO: Estado de Privacidad
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      ...
      .insert([{ 
        ...
        is_private: isPrivate // ENVIAMOS EL MODO PRIVADO
      }]);
    ...
    if (error) {
      ...
      setTitle(''); setSubjectId(''); setDate(''); setTime(''); setIsPrivate(false);
      onClose();
      onEventAdded();
    }
    setLoading(false);
        ...
        <div className="space-y-8">
          ...
          <div className={`flex items-center justify-between p-6 rounded-3xl border transition-all ${isPrivate ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-2xl ${isPrivate ? 'bg-orange-500 text-white' : 'bg-white text-slate-400'}`}>
                {isPrivate ? <EyeOff size={20} /> : <Lock size={20} />}
              </div>
              <div>
                <p className="text-sm font-black uppercase tracking-tight">Privacidad Inteligente</p>
                <p className={`text-[10px] font-medium ${isPrivate ? 'text-slate-400' : 'text-slate-500'}`}>Ocultar detalles al resto del nido</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <Input 
              ...
              className={`rounded-3xl h-16 border-none font-bold shadow-inner ${isPrivate ? 'bg-slate-800 text-white' : 'bg-slate-50/50 text-slate-700'}`} 
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <select 
                className={`w-full h-16 rounded-3xl border-none px-6 text-sm font-bold outline-none appearance-none ${isPrivate ? 'bg-slate-800 text-white' : 'bg-violet-50/30 text-[#8B5CF6]'}`} 
                value={subjectId} 
          ...
          <div className="grid grid-cols-2 gap-5">
            <div className="space-y-3">
              ...
              <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={`rounded-3xl h-16 border-none font-bold shadow-inner appearance-none ${isPrivate ? 'bg-slate-800 text-white' : 'bg-slate-50/50 text-slate-700'}`} />
            </div>
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] ml-2 flex items-center gap-2 opacity-50">
                <Clock size={14} /> Hora
              </label>
              <Input type="time" value={time} onChange={(e) => setTime(e.target.value)} className={`rounded-3xl h-16 border-none font-bold shadow-inner ${isPrivate ? 'bg-slate-800 text-white' : 'bg-slate-50/50 text-slate-700'}`} />
            </div>
          </div>
          ...
          >
            {loading ? "Sincronizando..." : isPrivate ? "Guardar en Privado" : "Confirmar en el Nido"}
          </Button>
        </div>
      </div>
    </div>
