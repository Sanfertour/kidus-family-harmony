...
export const ManualEventDrawer = ({ 
  ...
  useEffect(() => {
    const prepareDrawer = async () => {
      ...
      if (user) {
        ...
        if (profile?.nest_id) setCurrentNestId(profile.nest_id);
      }
    };
  ...
  const handleSave = async () => {
    if (!title || !date || !time || !currentNestId) {
      triggerHaptic('medium');
      toast({ 
        title: "Información incompleta", 
        description: !currentNestId ? "Error de sincronización con el Nido." : "Título, fecha y hora son obligatorios.", 
        variant: "destructive" 
      });
      return;
    }
    ...
    // Construcción de fecha ISO combinada
    ...
    if (!error) {
      ...
      toast({ title: "Evento Guardado", description: "Sincronía completada con éxito." });
      onClose();
      onEventAdded();
      // Reset local state
      setTitle('');
      ...
      <div className={`relative w-full max-w-md rounded-t-[3.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-500 max-h-[92vh] overflow-y-auto no-scrollbar ${isPrivate ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-800 border-t border-white'}`}>
        ...
        <div className="space-y-6 pb-12">
          ...
          <div className="flex justify-between gap-2">
            {EVENT_CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => { triggerHaptic('soft'); setCategory(cat.id); }} 
                className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-[2rem] border-2 transition-all ${category === cat.id ? `${cat.border} ${cat.bg} scale-105 shadow-md` : 'border-transparent bg-white opacity-40'}`}>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] ml-4 text-sky-500">Actividad</label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="¿Qué vamos a hacer?" className="rounded-[2rem] h-16 border-2 font-black text-lg px-8 focus:ring-0 bg-white border-slate-200 text-slate-800 shadow-sm" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="relative">
              ...
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full h-14 pl-12 pr-4 rounded-[1.5rem] border-2 border-slate-200 font-bold text-sm focus:outline-none bg-white text-slate-900" />
            </div>
            <div className="relative">
              <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-orange-500" size={16} />
              <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full h-14 pl-12 pr-4 rounded-[1.5rem] border-2 border-slate-200 font-bold text-sm focus:outline-none bg-white text-slate-900" />
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] ml-4 flex items-center gap-2 text-orange-500"><ListChecks size={14} /> Notas</label>
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ej: No olvidar las botas..." className="rounded-[2.5rem] min-h-[100px] border-2 border-slate-200 font-bold p-6 focus:ring-0 bg-white text-slate-800" />
          </div>
          ...
          <div className={`flex items-center justify-between p-5 rounded-[2.5rem] border-2 transition-all ${isPrivate ? 'bg-slate-900 border-orange-500/30' : 'bg-white border-slate-100 shadow-sm'}`}>
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] font-black uppercase">Modo Privado</span>
            </div>
            <Switch checked={isPrivate} onCheckedChange={(v) => { triggerHaptic('soft'); setIsPrivate(v); }} />
          </div>

          <Button onClick={handleSave} disabled={loading} className={`w-full h-20 rounded-[3rem] font-black text-lg tracking-[0.2em] shadow-2xl active:scale-95 transition-all mt-4 ${isPrivate ? 'bg-orange-500 hover:bg-orange-600' : 'bg-slate-800 text-white hover:bg-slate-900'}`}>
