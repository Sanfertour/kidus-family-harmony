...
export const AgendaView = () => {
  ...
  // 1. LÓGICA DE DATOS Y COLISIONES (LOGICA DEL 1)
    ...
    try {
      ...
      if (profile?.nest_id) {
        ...
        if (eventsData) {
          ...
          // Detectar solapes: mismo miembro, diferencia < 1 hora
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 font-sans selection:bg-sky-100 overflow-x-hidden">
      
      {/* HEADER ZEN */}
      <header className="pt-16 pb-12 px-10">
        <div className="flex justify-between items-start">
          ...
          <button 
            ...
            className="relative w-20 h-20 rounded-[2.5rem] bg-slate-800 text-white flex items-center justify-center shadow-2xl active:scale-90 transition-all duration-400"
          >
      ...
      {/* CALENDARIO GLASSMORPHISM (ESTETICA DEL 1) */}
      <section className="px-8 -mt-4">
        <div className="bg-white/60 backdrop-blur-2xl p-6 rounded-[3.5rem] border border-white shadow-xl shadow-slate-200/40">
            ...
            {weekDays.map((day, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-[13%] py-6 rounded-[2.2rem] flex flex-col items-center transition-all duration-500 ${active ? 'bg-slate-800 text-white shadow-2xl -translate-y-3' : 'text-slate-400'}`}
                >
      ...
      {/* EVENTOS CON LÓGICA DE SOLAPE Y EDICIÓN */}
          ...
          {events.filter(e => isSameDay(new Date(e.start_time), selectedDate)).length === 0 ? (
            ...
            events.filter(e => isSameDay(new Date(e.start_time), selectedDate)).map((event) => (
              <motion.div 
                ...
                className={`group relative p-8 rounded-[3.5rem] transition-all duration-500 border border-white bg-white shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 cursor-pointer overflow-hidden ${editingEventId === event.id ? 'ring-2 ring-sky-500/20' : ''}`}
              >
                ...
                {/* Edición contextual (FUNCIONALIDAD DEL 2) */}
                <AnimatePresence>
