...
export const SettingsView = () => {
  ...
  const handleLink = async () => {
    ...
    // Refuerzo de lógica: Aseguramos prefijo KID- para la DB
    const finalCode = cleanCode.startsWith('KID-') ? cleanCode : `KID-${cleanCode}`;
    
    // Llamada real al Store (que conecta con Supabase RPC)
  ...
  return (
    <div className="w-full space-y-4 px-4 pb-20">
      ...
      <div className="space-y-3">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 ml-4">Comunidad</p>
      ...
      <div className="space-y-3">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 ml-4">Nido</p>
      ...
      {/* MODAL DE VINCULACIÓN (PORTAL DE SINCRONÍA) */}
      <AnimatePresence>
        {isLinking && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[150] flex items-center justify-center px-6 bg-slate-900/20 backdrop-blur-md"
          >
            <motion.div 
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="w-full max-w-sm bg-white/80 backdrop-blur-3xl p-8 rounded-[3.5rem] border border-white shadow-2xl relative"
            >
              <button 
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-3 italic">Introduce el código único</p>
              </div>
              ...
              <div className="space-y-4">
                <input
                  ...
                  className="w-full bg-white/40 border border-white/60 px-6 py-5 rounded-[2rem] text-center text-xl font-black tracking-widest text-slate-900 focus:outline-none focus:ring-4 focus:ring-sky-500/10 transition-all shadow-inner placeholder:text-slate-300"
                />
                ...
                >
                  {status === 'loading' ? <Loader2 className="animate-spin" size={18} /> : 
                   ...
                   status === 'error' ? <AlertCircle size={18} /> : "Unirse al Nido"}
                </button>
                
                {status === 'error' && (
                  <p className="text-center text-[10px] font-black text-red-500 uppercase tracking-widest animate-pulse">Código no válido</p>
                )}
              </div>
            </motion.div>
          </motion.div>
