...
// Exportación nominal para que el Build de Vercel no falle
export const SettingsView = () => {
  ...
  // Lógica de Sincronía de Nido Real (Reforzada)
  const handleLink = async () => {
    ...
    // Aseguramos el prefijo KID- para la base de datos
    const finalCode = cleanCode.startsWith('KID-') ? cleanCode : `KID-${cleanCode}`;
    
    // Llamada funcional al backend para cambiar de Nido
  ...
  return (
    <div className="w-full space-y-4">
      {/* BOTÓN 1: NUEVO MIEMBRO (Funcionalidad de adición mantenida) */}
      <motion.button
        whileTap={{ scale: 0.95 }}
      ...
      {/* BOTÓN 2: AJUSTES (Mantenido según tu diseño) */}
      <motion.button
        whileTap={{ scale: 0.95 }}
      ...
      {/* MODAL DE VINCULACIÓN: Aquí ocurre la sincronía de nidos por código */}
      <AnimatePresence>
        {isLinking && (
          <motion.div 
            initial={{ opacity: 0 }}
